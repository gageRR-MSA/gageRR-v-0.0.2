...
$sql = "SELECT * FROM measureloop WHERE GageName='$_POST[gn]' AND Part='$p' AND Appraiser='$a' AND Trial='$t'";
$result = mysql_query($sql,$con);
if ($myrow = mysql_fetch_array($result)){
do {

$_POST["r$p$a$t"]=$myrow["answer"];
...
for($a=1;$a<=$appraiser;$a++){
?>
<td bgcolor="#D8D8D8"> <div align="center"><strong>Appraiser <?php printf("%s",$a); ?></strong></div></td>


<?php
}
?>
<td width="200"bgcolor="#D8D8D8" rowspan="2"><div align="center"><strong><u>X</u></strong></div></td></tr>

<tr><td bgcolor="#D8D8D8"><div align="center"><strong>Operator</strong></div></td>
<?php
//display table first row base on gage requirement
for($a=1;$a<=$appraiser;$a++){
	?>
...
?>
<tr>
<td bgcolor="#D8D8D8"><strong>Part <?php printf("%s",$p); ?></strong></td>
<?php
$sum=0;
$calmean[$p]=0;
for($a=1;$a<=$appraiser;$a++){
	...
	?>
    <input type="text" style="width:10mm" style="text-align:center" name="r<?php printf("%s%s%s",$p,$a,$t); ?>" value="<?php printf("%s",$_POST["r$p$a$t"]); ?>">
...
?>
  <input type="text" style="text-align:center"style="width:10mm" name="s<?php printf("%s%s%s",$p,$a,$t); ?>5" value="<?php printf("%s",number_format($range,4, '.', '')); ?>" disabled="disabled">
  <input type="text" style="text-align:center"style="width:10mm" name="a<?php printf("%s%s%s",$p,$a,$t); ?>3" value="<?php printf("%s",number_format($avg,4, '.', '')); ?>" disabled="disabled">
  <input type="hidden" style="text-align:center"style="width:10mm" name="s<?php printf("%s%s%s",$p,$a,$t); ?>5" value="<?php printf("%s",number_format($range,4, '.', '')); ?>">
  <input type="hidden" style="text-align:center"style="width:10mm" name="a<?php printf("%s%s%s",$p,$a,$t); ?>3" value="<?php printf("%s",number_format($avg,4, '.', '')); ?>">
  </td>
...
?>
<td><input type="text" style="text-align:center"style="width:54mm" name="mean<?php printf("%s%s",$p,$a,$t); ?>" value="<?php printf("%s",number_format($calmean[$p],4, '.', '')); ?>" disabled="disabled"></td>
</tr>
<?php
}
?>
...
<tr>
<td bgcolor="#D8D8D8"><strong>Total </strong></td>
...
<input type="text" style="text-align:center"style="width:<?php printf("%s",$size); ?>mm" name="total<?php printf("%s",$a); ?>" value="<?php printf("%s",number_format($totalsum[$a],4, '.', '')); ?>" disabled="disabled">
...
<input type="text" style="text-align:center"style="width:10mm" name="midrange<?php printf("%s",$a); ?>" value="<?php printf("%s",number_format($Ra[$a],4, '.', '')); ?>" disabled="disabled">
<input type="text" style="text-align:center"style="width:10mm" name="midavg<?php printf("%s",$a); ?>" value="<?php printf("%s",number_format($Xa[$a],4, '.', '')); ?>" disabled="disabled"></td>
<?php
}
?>
...
$range2=$_POST["$firt"];
$avg2=$_POST["$sec"];
$sql="INSERT INTO measureloop4 (GageName,Part,Appraiser,TotalRange,TotalAvg)
VALUES
('$_POST[gn]','$p','$a','$range2','$avg2')";
...
?>
  <script>alert("The values have been saved successfully in <?php echo "$_POST[gn]" ?> ! Please proceed to the Metrics page");</script>
  <?php
}
}
?>